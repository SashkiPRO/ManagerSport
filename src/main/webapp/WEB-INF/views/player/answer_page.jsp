              ...
              <table class="table table-striped">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>${message.event.hall.name}</td>
                    <td>${message.event.dateString}</td>
                    <td>
                    <c:if test="${message.status=='read'}"><i class="glyphicon glyphicon-ok-circle icon-style"></i></c:if>
                    <c:if test="${message.status=='unread'}"><i class="glyphicon glyphicon-time"></i></c:if>
                    <c:if test="${message.status=='disagree'}"><i class="glyphicon glyphicon-remove-circle"></i></c:if>

                    </td>
                  </tr>
                <c:if test="${message.status=='read'}">
                  <tr><td></td><td colspan="2"><a href="accept_event?id=${message.id}" class="btn btn-info disabled">Принять</a> </td><td colspan="2"><a href="disagree_event?id=${message.id}" class="btn btn-info">Отклонить</a></td><td></td></tr>
                </c:if>
                  <c:if test="${message.status=='unread'}">
                    <tr><td></td><td colspan="2"><a href="accept_event?id=${message.id}" class="btn btn-info">Принять</a></td><td colspan="2"> <a href="disagree_event?id=${message.id}" class="btn btn-info">Отклонить</a> </td><td></td></tr>
                  </c:if>
                  <c:if test="${message.status=='disagree'}">
                    <tr><td></td><td colspan="2"> <a href="accept_event?id=${message.id}" class="btn btn-info">Принять</a></button> </td><td colspan="2"><a href="disagree_event?id=${message.id}" class="btn btn-info disabled">Отклонить</a></td><td></td></tr>
                  </c:if>
                </tbody>
              </table>
            </div>
